          ...
          <div className="w-full max-w-sm space-y-4 bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl">
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full p-4 rounded-lg bg-slate-950 border border-slate-700 text-white focus:border-emerald-500 outline-none"
                onChange={(e) => setTeamId(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleLogin()}
              />
            </div>
            <button onClick={handleLogin} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white p-4 rounded-lg font-bold shadow-lg">
              Enter Boardroom
            </button>
          </div>
        </div>
...
function Dashboard({ teamId, onLogout }) {
  ...
  const submitChoice = async (tier, cost, debt) => {
    ...
    if(!confirm(`Confirm ${tier}?`)) return;
    setLoading(true)
        ...
        <div>
            ...
            {isLocked ? (
                ...
                <div className="space-y-3">
                    <StrategyBtn title="Tier A (Ethical)" price={1200} debt={-1} debtColor="text-emerald-400" onClick={() => submitChoice('Tier A (Ethical)', 1200, -1)} loading={loading} />
                    <StrategyBtn title="Tier B (Standard)" price={800} debt={1} debtColor="text-orange-400" onClick={() => submitChoice('Tier B (Standard)', 800, 1)} loading={loading} />
                    <StrategyBtn title="Tier C (Dirty)" price={500} debt={3} debtColor="text-red-500" onClick={() => submitChoice('Tier C (Dirty)', 500, 3)} loading={loading} />
                </div>
            )}
        </div>
      </div>
